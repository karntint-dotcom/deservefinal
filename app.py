...
if uploaded:
    ...
    if st.button("🚀 สร้าง Dashboard", type="primary", use_container_width=True):
        ...
        try:
            ...
            output_bytes, latest_date = build_dashboard(
            ...
            # ── ตั้งชื่อไฟล์: Deserve Dashboard_30JunUpdate ──────────────────
            if latest_date is not None and pd.notna(latest_date):
                date_str = latest_date.strftime("%-d%b") if hasattr(latest_date, "strftime") else ""
                # Linux strftime %-d ไม่รองรับบน Windows — ใช้ fallback
                try:
                    date_str = f"{latest_date.day}{latest_date.strftime('%b')}"
                except Exception:
                    date_str = latest_date.strftime("%d%b")
                fname = f"Deserve Dashboard_{date_str}Update.xlsx"
            else:
