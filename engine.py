...
def load_data(file_bytes):
    ...
    buf.seek(0); df_all = pd.read_excel(buf, sheet_name=data_sheet, header=1)
    for col in ['ชื่อลูกค้า','รหัสลูกค้า','รหัสสินค้า','ชื่อสินค้า','หมวดหมู่','สถานะรายการ']:
        if col in df_all.columns: df_all[col] = df_all[col].astype(str).str.strip()
    df_all['ราคารวม'] = pd.to_numeric(df_all['ราคารวม'], errors='coerce').fillna(0)
    df_all['จำนวน']   = pd.to_numeric(df_all['จำนวน'],   errors='coerce').fillna(0)
    df_all['date']    = pd.to_datetime(df_all['วันที่ทำรายการ'], dayfirst=True, errors='coerce')
    df_all['month']   = df_all['date'].dt.month

    # เก็บรายการที่ "ไม่สำเร็จ" ไว้แยกต่างหาก (สำหรับ Sheet 17)
    df_pending = df_all[df_all['สถานะรายการ'] != 'สำเร็จ'].copy()

    df = df_all[df_all['สถานะรายการ'] == 'สำเร็จ'].copy()
    ...
    # Apply same mappings to df_pending (สำหรับ Sheet 17)
    if not df_pending.empty:
        df_pending['ชื่อร้าน'] = df_pending['ชื่อลูกค้า'].map(n2s).fillna(df_pending['ชื่อลูกค้า'])
        df_pending['รหัสลูกค้า_norm'] = df_pending['รหัสลูกค้า'].map(lambda x: cm.get(x, x))
    ...
    for grp, gdf in df.groupby('prod_grp'):
        ...
        m_ = gdf['หมวดหมู่'].mode(); prod_cat[grp] = m_.iloc[0] if len(m_)>0 else ''

    return df, prod_names, prod_cat, monthly_targets_from_file, df_pending
...
def make_forecast(vals, months_act, months_fc):
    ...
    tail = vals[-3:]; n = len(tail)
    w = [1,2,3][-n:] if n > 0 else [1]
    wma = float(max(0, np.average(tail, weights=w) if n > 0 else 0))
    return sl, r2, fc, wma
...
def build_sheet2(wb, df, grand_total, months):
    ws = wb.create_sheet("4_Ranking_ร้านค้า"); ws.sheet_properties.tabColor = "2E75B6"
    W(ws,1,1,"Ranking ยอดขายรายเดือน – ชื่อร้าน",bg="1F4E79",fg="FFFFFF",b=True,sz=12,h="left",merge_to=5+len(months))
    ...
    # ── รหัสลูกค้า (ตัวอย่าง) ต่อร้าน ────────────────────────────────────────
    code_info = df.groupby('ชื่อร้าน')['รหัสลูกค้า_norm'].agg(lambda x: list(x.unique()))
    def sample_code(store):
        codes = code_info.get(store, [])
        if not codes: return ''
        if len(codes) == 1: return codes[0]
        return f"{codes[0]}  (+{len(codes)-1} สาขา)"

    hdrs = ['อันดับ','ชื่อร้าน','รหัสลูกค้า (ตัวอย่าง)']+[MTH[m] for m in months]+['รวม (บาท)','เฉลี่ย/เดือน','%ยอดรวม','สถานะ']
    ...
    for ri_off,(store,row) in enumerate(sm.iterrows(),1):
        ...
        W(ws,ri,3,sample_code(store),bg=alt,h="left",sz=8,italic=True)
        ...
        W(ws,ri,7+len(months),"⚠️ เฝ้าระวัง" if d>=2 else("📊 ปกติ" if d==1 else "📈 ดี"),bg=sb,b=True,sz=8)
    ...
    for i in range(4,4+len(months)+5): ws.column_dimensions[gc(i)].width=12
    ws.freeze_panes="D4"
...
def build_sheet3(wb, df, grand_total, months):
    ws = wb.create_sheet("5_Ranking_รหัสลูกค้า"); ws.sheet_properties.tabColor = "375623"
    ...
    code_store = df.groupby('รหัสลูกค้า_norm')['ชื่อร้าน'].agg(lambda x: x.mode().iloc[0] if len(x.mode())>0 else x.iloc[0])
...
def build_sheet4(wb, df, grand_total, months):
    ws = wb.create_sheet("11_ยอดขายฟาร์ม"); ws.sheet_properties.tabColor = "375623"
...
def build_sheet5(wb, df, grand_total, months, plc_target, plc_deadline, recco_target, recco_deadline):
    ws = wb.create_sheet("12_เป้าหมาย_PLC_Recco"); ws.sheet_properties.tabColor = "C00000"
...
def build_sheet10(wb, df, grand_total, months):
    ...
    ws=wb.create_sheet("13_Forecast_ร้านค้า"); ws.sheet_properties.tabColor="1F4E79"
...
def build_sheet11(wb, df, grand_total, months, prod_names, prod_cat):
    ws=wb.create_sheet("14_Forecast_สินค้า"); ws.sheet_properties.tabColor="7030A0"
...
def build_sheet13(wb, df, months, prod_names, prod_cat):
    ...
    """
    ws = wb.create_sheet("10_Matrix_ชิ้น×ลูกค้า"); ws.sheet_properties.tabColor = "0070C0"
...
def build_sheet12(wb, df, grand_total, months, monthly_targets):
    ws=wb.create_sheet("3_เป้าหมายรายเดือน"); ws.sheet_properties.tabColor="FF0000"
...
# ════════════════════════════════════════════════════════════════════════════
# SHEET 15: ยอดขายแยกตัวแทน (Sales Rep)
# ════════════════════════════════════════════════════════════════════════════
def build_sheet15(wb, df, grand_total, months):
    """สร้าง 2 sheets: สรุป + เปรียบเทียบ"""
    if 'ชื่อตัวแทน' not in df.columns: return
    df = df.copy()
    df['ชื่อตัวแทน'] = df['ชื่อตัวแทน'].astype(str).str.strip()
    df['prod_grp'] = df.get('prod_grp', df['รหัสสินค้า'].str[:6])

    # รวม Sale แจ๊ค เข้ากับ บริษัท
    REP_REMAP = {'Sale แจ๊ค': 'บริษัท'}
    df['ชื่อตัวแทน'] = df['ชื่อตัวแทน'].map(lambda x: REP_REMAP.get(x, x))

    REP_CLR = {
        'Sale น้ำส้ม': ('FF8C00','FFF3E0','น้ำส้ม'),
        'อุ้ม':         ('0070C0','E3F2FD','อุ้ม'),
        'บริษัท':       ('595959','F2F2F2','บริษัท'),
    }
    REPS = [r for r in REP_CLR if r in df['ชื่อตัวแทน'].values]
    if not REPS: return

    T_ = Side(style='thin', color='D0D0D0')
    BT_ = Border(left=T_,right=T_,top=T_,bottom=T_)

    def F_(h): return PatternFill('solid',fgColor=h)
    def W_(ws_,r,c,val,bg=None,fg='000000',b=False,sz=9,fmt=None,
           h='center',wrap=False,border=BT_,merge_to=None):
    ...
    # ── Sheet A: สรุปยอดขายต่อตัวแทน ────────────────────────────────────────
    ws1=wb.create_sheet('15_ยอดขายต่อตัวแทน'); ws1.sheet_properties.tabColor='1F4E79'
    W_(ws1,1,1,'ยอดขายแยกตามตัวแทน',bg='1F4E79',fg='FFFFFF',b=True,sz=13,
       h='left',merge_to=4+len(months))
    ws1.row_dimensions[1].height=26

    for ci,h_ in enumerate(['ตัวแทน']+[MTH[m] for m in months]+['รวม (บาท)','สัดส่วน','จำนวนร้าน'],1):
        W_(ws1,3,ci,h_,bg='1F4E79',fg='FFFFFF',b=True,sz=9,wrap=True)
    ws1.row_dimensions[3].height=28

    ri=4
    for rep in REPS:
        cd,cl,lbl=REP_CLR[rep]
        sub=df[df['ชื่อตัวแทน']==rep]
        total=sub['ราคารวม'].sum()
        ws1.row_dimensions[ri].height=20
        W_(ws1,ri,1,lbl,bg=cd,fg='FFFFFF',b=True,sz=11)
        for ci,m in enumerate(months,2):
            W_(ws1,ri,ci,sub[sub['month']==m]['ราคารวม'].sum(),bg=cl,b=True,fmt='#,##0.00')
        W_(ws1,ri,2+len(months),total,bg=cd,fg='FFFFFF',b=True,fmt='#,##0.00',sz=11)
        W_(ws1,ri,3+len(months),total/grand_total if grand_total else 0,bg=cl,b=True,fmt='0.0%')
        W_(ws1,ri,4+len(months),sub['ชื่อลูกค้า'].nunique(),bg=cl,b=True)
        ri+=1

    # Total
    ws1.row_dimensions[ri].height=18
    W_(ws1,ri,1,'รวม (ระบุตัวแทน)',bg='2E75B6',fg='FFFFFF',b=True)
    df_rep = df[df['ชื่อตัวแทน'].isin(REPS)]
    for ci,m in enumerate(months,2):
        W_(ws1,ri,ci,df_rep[df_rep['month']==m]['ราคารวม'].sum(),bg='2E75B6',fg='FFFFFF',b=True,fmt='#,##0.00')
    W_(ws1,ri,2+len(months),df_rep['ราคารวม'].sum(),bg='1F4E79',fg='FFFFFF',b=True,fmt='#,##0.00',sz=11)
    ri+=2

    # Ranking ร้านค้าต่อตัวแทน
    for rep in REPS:
        cd,cl,lbl=REP_CLR[rep]
        sub=df[df['ชื่อตัวแทน']==rep]
        if sub.empty: continue
        total_rep=sub['ราคารวม'].sum()

        W_(ws1,ri,1,f'📊 Ranking ร้าน — {lbl}',bg=cd,fg='FFFFFF',b=True,sz=10,
           h='left',merge_to=4+len(months))
        ws1.row_dimensions[ri].height=18; ri+=1

        for ci,h_ in enumerate(['อันดับ','ชื่อลูกค้า']+[MTH[m] for m in months]+['รวม (บาท)','%ของตัวแทน'],1):
            W_(ws1,ri,ci,h_,bg=cd,fg='FFFFFF',b=True,sz=8,wrap=True)
        ws1.row_dimensions[ri].height=28; ri+=1

        sp=sub.groupby(['ชื่อลูกค้า','month'])['ราคารวม'].sum().unstack(fill_value=0)
        for m in months:
            if m not in sp.columns: sp[m]=0
        sp=sp[months]; sp['รวม']=sp.sum(axis=1); sp=sp.sort_values('รวม',ascending=False)

        for ri_off,(store,row) in enumerate(sp.iterrows(),1):
            alt=cl if ri_off%2==0 else None; ws1.row_dimensions[ri].height=14
            W_(ws1,ri,1,ri_off,bg=alt)
            W_(ws1,ri,2,store,bg=alt,h='left',sz=8)
            for ci,m in enumerate(months,3):
                v=row.get(m,0); W_(ws1,ri,ci,v,bg='FFF0F0' if v==0 else alt,fmt='#,##0',sz=8)
            W_(ws1,ri,3+len(months),row['รวม'],bg=cl,b=True,fmt='#,##0.00')
            W_(ws1,ri,4+len(months),row['รวม']/total_rep if total_rep else 0,bg=alt,fmt='0.0%',sz=8)
            ri+=1
        ri+=1

    ws1.column_dimensions['A'].width=20
    for i in range(2,6+len(months)): ws1.column_dimensions[gc(i)].width=13
    ws1.freeze_panes='C4'

    # ── Sheet B: เปรียบเทียบ น้ำส้ม vs อุ้ม ─────────────────────────────────
    ns_key='Sale น้ำส้ม'; um_key='อุ้ม'
    if ns_key not in df['ชื่อตัวแทน'].values and um_key not in df['ชื่อตัวแทน'].values:
        return

    ws2=wb.create_sheet('16_เปรียบเทียบตัวแทน'); ws2.sheet_properties.tabColor='375623'
    W_(ws2,1,1,'เปรียบเทียบยอดขาย น้ำส้ม vs อุ้ม',bg='1F4E79',fg='FFFFFF',b=True,sz=13,
       h='left',merge_to=4)
    ws2.row_dimensions[1].height=26

    ns=df[df['ชื่อตัวแทน']==ns_key]
    um=df[df['ชื่อตัวแทน']==um_key]
    ns_total=ns['ราคารวม'].sum(); um_total=um['ราคารวม'].sum()

    # KPI
    for i,(lbl2,val,fmt_,bg_) in enumerate([
        ('น้ำส้ม รวม',ns_total,'#,##0.00','FFF3E0'),
        ('อุ้ม รวม',um_total,'#,##0.00','E3F2FD'),
        ('ต่าง (น้ำส้ม-อุ้ม)',ns_total-um_total,'+#,##0;-#,##0;0','E2EFDA' if ns_total>=um_total else 'FFD7D7'),
    ]):
        W_(ws2,3,i*2+1,lbl2,bg='2E75B6',fg='FFFFFF',b=True,sz=9)
        W_(ws2,4,i*2+1,val,bg=bg_,b=True,sz=12,fg='1F4E79',fmt=fmt_,merge_to=i*2+2)
    ws2.row_dimensions[4].height=24

    for ci,h_ in enumerate(['เดือน','น้ำส้ม (บาท)','อุ้ม (บาท)','ต่าง','% น้ำส้ม','% อุ้ม'],1):
        W_(ws2,6,ci,h_,bg='1F4E79',fg='FFFFFF',b=True,sz=9,wrap=True)
    ws2.row_dimensions[6].height=28

    all_months_cmp = sorted(set(list(months)+list(um['month'].dropna().unique().astype(int))))
    for ri_off,m in enumerate(all_months_cmp,1):
        ri_=ri_off+6; alt='F0F4FF' if ri_off%2==0 else None
        ns_v=ns[ns['month']==m]['ราคารวม'].sum()
        um_v=um[um['month']==m]['ราคารวม'].sum()
        diff=ns_v-um_v
        dbg='E2EFDA' if diff>=0 else 'FFD7D7'
        W_(ws2,ri_,1,MTH.get(m,''),bg=alt,b=True)
        W_(ws2,ri_,2,ns_v if ns_v>0 else '—',bg='FFF3E0' if ns_v>0 else alt,
           fmt='#,##0.00' if ns_v>0 else None)
        W_(ws2,ri_,3,um_v if um_v>0 else '—',bg='E3F2FD' if um_v>0 else alt,
           fmt='#,##0.00' if um_v>0 else None)
        W_(ws2,ri_,4,diff if (ns_v>0 or um_v>0) else '—',bg=dbg,
           fmt='+#,##0;-#,##0;0' if (ns_v>0 or um_v>0) else None)
        W_(ws2,ri_,5,ns_v/ns_total if ns_total and ns_v>0 else '—',bg=alt,
           fmt='0.0%' if ns_v>0 else None,sz=8)
        W_(ws2,ri_,6,um_v/um_total if um_total and um_v>0 else '—',bg=alt,
           fmt='0.0%' if um_v>0 else None,sz=8)

    tr_=len(all_months_cmp)+7
    W_(ws2,tr_,1,'รวม',bg='1F4E79',fg='FFFFFF',b=True)
    W_(ws2,tr_,2,ns_total,bg='FF8C00',fg='FFFFFF',b=True,fmt='#,##0.00')
    W_(ws2,tr_,3,um_total,bg='0070C0',fg='FFFFFF',b=True,fmt='#,##0.00')
    W_(ws2,tr_,4,ns_total-um_total,
       bg='E2EFDA' if ns_total>=um_total else 'FFD7D7',b=True,fmt='+#,##0;-#,##0;0')
    for i,w_ in enumerate([10,16,16,16,12,12],1): ws2.column_dimensions[gc(i)].width=w_


# ════════════════════════════════════════════════════════════════════════════
# SHEET 17: รายการรอโอน / ไม่สำเร็จ
# ════════════════════════════════════════════════════════════════════════════
def build_sheet17(wb, df_pending):
    """แสดงรายการที่ยังไม่ใช่สถานะ 'สำเร็จ' — เช่น รอโอน"""
    if df_pending is None or df_pending.empty: return

    ws = wb.create_sheet("17_รายการรอโอน"); ws.sheet_properties.tabColor = "FFC000"

    pending = df_pending.copy()
    if 'ชื่อร้าน' not in pending.columns: pending['ชื่อร้าน'] = pending['ชื่อลูกค้า']
    if 'รหัสลูกค้า_norm' not in pending.columns: pending['รหัสลูกค้า_norm'] = pending['รหัสลูกค้า']
    pending['prod_grp'] = pending['รหัสสินค้า'].str[:6]

    total_pending = pending['ราคารวม'].sum()
    n_orders = len(pending)
    n_stores = pending['ชื่อร้าน'].nunique()

    STATUS_CLR = {
        'รอโอน':   ('FFC000','FFF8E1'),
        'ยกเลิก':  ('C00000','FCE4D6'),
        'nan':     ('808080','F2F2F2'),
    }

    W(ws,1,1,"⏳ รายการที่ยังไม่สำเร็จ (รอโอน / สถานะอื่น)",
      bg="FFC000",fg="FFFFFF",b=True,sz=13,h="left",merge_to=7)
    ws.row_dimensions[1].height=26
    W(ws,2,1,"💡 รายการเหล่านี้ไม่ถูกนับรวมในยอดขายของ Dashboard sheet อื่นๆ ทั้งหมด",
      bg="FFF8E1",fg="7B5E00",sz=8,h="left",merge_to=7)
    ws.row_dimensions[2].height=14

    # KPI
    for i,(lbl,val,fmt) in enumerate([
        ("ยอดรอโอน (บาท)", total_pending, '#,##0.00'),
        ("จำนวนรายการ", n_orders, '#,##0'),
        ("จำนวนร้าน", n_stores, '#,##0'),
    ]):
        W(ws,4,i*2+1,lbl,bg="FFC000",fg="FFFFFF",b=True,sz=9)
        W(ws,5,i*2+1,val,bg="FFF8E1",b=True,sz=12,fg="7B5E00",fmt=fmt,merge_to=i*2+2)
    ws.row_dimensions[5].height=24

    # ── สรุปตามสถานะ ─────────────────────────────────────────────────────────
    W(ws,7,1,"สรุปตามสถานะ",bg="FFC000",fg="FFFFFF",b=True,sz=10,h="left",merge_to=4)
    ws.row_dimensions[7].height=18
    for ci,h in enumerate(['สถานะ','จำนวนรายการ','ยอดรวม (บาท)'],1):
        W(ws,8,ci,h,bg="7B5E00",fg="FFFFFF",b=True,sz=9)
    ws.row_dimensions[8].height=18

    status_summary = pending.groupby('สถานะรายการ').agg(
        n=('ราคารวม','count'), total=('ราคารวม','sum')).sort_values('total',ascending=False)
    ri = 9
    for status, row in status_summary.iterrows():
        sbg, scl = STATUS_CLR.get(status, ('595959','F2F2F2'))
        W(ws,ri,1,status if status!='nan' else '(ไม่ระบุ)',bg=scl,fg=sbg,b=True,sz=9)
        W(ws,ri,2,int(row['n']),bg=scl,sz=9)
        W(ws,ri,3,row['total'],bg=scl,fmt='#,##0.00',sz=9)
        ri+=1

    # ── ตารางรายการละเอียด ──────────────────────────────────────────────────
    ri += 1
    W(ws,ri,1,"📋 รายการละเอียด",bg="FFC000",fg="FFFFFF",b=True,sz=10,h="left",merge_to=7)
    ws.row_dimensions[ri].height=18; ri+=1

    hdr_row = ri
    for ci,h in enumerate(['วันที่','ชื่อร้าน','รหัสลูกค้า','ชื่อสินค้า','จำนวน','ราคารวม (บาท)','สถานะ'],1):
        W(ws,hdr_row,ci,h,bg="7B5E00",fg="FFFFFF",b=True,sz=8,wrap=True)
    ws.row_dimensions[hdr_row].height=22
    ri+=1

    pending_sorted = pending.sort_values('date', ascending=False)
    for ri_off,(_,row) in enumerate(pending_sorted.iterrows(),1):
        r = ri+ri_off-1
        alt = "FFFBF0" if ri_off%2==0 else None
        ws.row_dimensions[r].height=14
        status = row.get('สถานะรายการ','')
        sbg, scl = STATUS_CLR.get(status, ('595959','F2F2F2'))

        date_val = row['date']
        date_str = date_val.strftime('%d/%m/%Y') if pd.notna(date_val) else '—'
        W(ws,r,1,date_str,bg=alt,sz=8)
        W(ws,r,2,row.get('ชื่อร้าน',''),bg=alt,h="left",sz=8)
        W(ws,r,3,row.get('รหัสลูกค้า_norm',''),bg=alt,h="left",sz=8)
        W(ws,r,4,row.get('ชื่อสินค้า',''),bg=alt,h="left",sz=8,wrap=False)
        W(ws,r,5,row.get('จำนวน',0),bg=alt,fmt='#,##0',sz=8)
        W(ws,r,6,row.get('ราคารวม',0),bg=alt,fmt='#,##0.00',sz=8)
        W(ws,r,7,status if status!='nan' else '(ไม่ระบุ)',bg=scl,fg=sbg,b=True,sz=8)

    tr = ri+len(pending_sorted)
    W(ws,tr,5,"รวม",bg="FFC000",fg="FFFFFF",b=True,h="right")
    W(ws,tr,6,total_pending,bg="FFC000",fg="FFFFFF",b=True,fmt='#,##0.00')
...
def build_dashboard(file_bytes, plc_target=1_250_000, plc_deadline="30 มิถุนายน 2569",
    ...
    upd(8,  "📥 โหลดข้อมูล...")
    df, prod_names, prod_cat, targets_from_file, df_pending = load_data(file_bytes)
    ...
    # ── 1. ภาพรวมยอดขาย ──────────────────────────────────────────────────────
    upd(15, "📊 ภาพรวมยอดขาย..."); build_sheet1(wb, df, grand_total, months, monthly_targets)

    # ── 2. update รายสัปดาห์ + 3. เป้าหมายรายเดือน (ต้องมี monthly_targets) ──
    if monthly_targets:
        upd(22, "📅 ยอดขายรายสัปดาห์..."); build_sheet14(wb, df, months, monthly_targets)
        upd(28, "🎯 เป้าหมายรายเดือน...");  build_sheet12(wb, df, grand_total, months, monthly_targets)

    # ── 4. Ranking ร้านค้า / 5. Ranking รหัสลูกค้า ──────────────────────────
    upd(36, "🏪 Ranking ร้านค้า...");   build_sheet2(wb, df, grand_total, months)
    upd(42, "🔑 Ranking รหัสลูกค้า..."); build_sheet3(wb, df, grand_total, months)

    # ── 6. ภาพรวมสินค้า / 7. Ranking สินค้า ────────────────────────────────
    upd(50, "📦 ภาพรวมสินค้า (+ ชิ้น)...");     build_sheet6(wb, df, grand_total)
    upd(58, "📋 Ranking สินค้า (+ ชิ้น)...");    build_sheet7(wb, df, grand_total, months, prod_names, prod_cat)

    # ── 8-9. สินค้า × Top10 ร้าน/รหัส ───────────────────────────────────────
    upd(66, "🔗 สินค้า × Top10 (+ ชิ้น)..."); build_sheets_89(wb, df, grand_total, months, prod_names, prod_cat)

    # ── 10. Matrix ชิ้น × ลูกค้า ─────────────────────────────────────────────
    upd(74, "📊 Matrix ชิ้น × ลูกค้า..."); build_sheet13(wb, df, months, prod_names, prod_cat)

    # ── 11. ยอดขายฟาร์ม / 12. เป้าหมาย PLC&Recco ───────────────────────────
    upd(80, "🌾 ยอดขายฟาร์ม...");
    build_sheet4(wb, df, grand_total, months)
    upd(85, "🎯 เป้าหมาย PLC & Recco...")
    build_sheet5(wb, df, grand_total, months, plc_target, plc_deadline, recco_target, recco_deadline)

    # ── 13-14. Forecast ร้านค้า / สินค้า ────────────────────────────────────
    upd(90, "🔮 Forecast ร้านค้า..."); build_sheet10(wb, df, grand_total, months)
    upd(95, "📦 Forecast สินค้า × ร้าน (+ ชิ้น)..."); build_sheet11(wb, df, grand_total, months, prod_names, prod_cat)

    # ── 15-16. ยอดขายต่อตัวแทน / เปรียบเทียบตัวแทน ─────────────────────────
    upd(96, "👥 ยอดขายแยกตัวแทน..."); build_sheet15(wb, df, grand_total, months)

    # ── 17. รายการรอโอน / ไม่สำเร็จ ─────────────────────────────────────────
    upd(98, "⏳ รายการรอโอน..."); build_sheet17(wb, df_pending)

    upd(99, "💾 บันทึกไฟล์...")
    out = io.BytesIO(); wb.save(out); out.seek(0)

    # หาวันที่ล่าสุดในข้อมูล (ทุกสถานะ) สำหรับตั้งชื่อไฟล์
    dates_all = []
    if 'date' in df.columns and df['date'].notna().any():
        dates_all.append(df['date'].max())
    if df_pending is not None and not df_pending.empty and 'date' in df_pending.columns and df_pending['date'].notna().any():
        dates_all.append(df_pending['date'].max())
    latest_date = max(dates_all) if dates_all else None

    return out.read(), latest_date

# ════════════════════════════════════════════════════════════════════════════
# SHEET 14: ยอดขายรายสัปดาห์
# ════════════════════════════════════════════════════════════════════════════
def build_sheet14(wb, df, months, monthly_targets):
    ws = wb.create_sheet("2_update รายสัปดาห์")
